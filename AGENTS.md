...
- **Stack:** Phaser 4 (Beam WebGL2), Capacitor, Node 22+, pnpm, Turborepo 2.x, Vitest. Never use Phaser 3 memory. Use Context7 for v4 APIs.
...
- **Ads/UMP:** All ad calls go through `@blublux/ads-adapter`; **UMP gates loading** — `initialize()` → `requestConsentInfo()` → `showConsentForm()` if required → check `canRequestAds` → only then load/show. Test ad unit IDs wired to `import.meta.env.DEV`.
- **Ad-UX policy:** Never launch/exit/back-to-back/mid-gameplay; preload at level start, show at level end; mute + disable input while shown; rewarded opt-in + grant only on the verified completion event; first-session grace.
